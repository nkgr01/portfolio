    ...
    <section id="hero" className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden px-6 pt-28 pb-16">
      {floatingTags.map((tag, i) => (
        <motion.div key={i} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 0.6, scale: 1, y: [0, -15, 0] }}
          ...
          className="absolute hidden lg:flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white/[0.05] border border-white/[0.1] backdrop-blur-sm text-xs text-white/60"
          style={{ top: tag.top, left: tag.left, right: tag.right }} whileHover={{ scale: 1.1, opacity: 1 }}>
          <span className="text-[#4ade80]/70 font-semibold">+</span>{tag.label}
        </motion.div>
      ))}

      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-[radial-gradient(ellipse_at_center,rgba(74,222,128,0.08)_0%,transparent_70%)] pointer-events-none" />
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="relative z-10 text-center max-w-3xl mx-auto">
        <motion.div variants={itemVariants} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[#4ade80]/10 border border-[#4ade80]/20 text-sm text-white/75 mb-8">
        ...
        <motion.h1 variants={itemVariants} className="font-display font-extrabold text-[clamp(3rem,10vw,7rem)] leading-[0.95] tracking-tighter text-white">N'GUESSAN</motion.h1>
        <motion.h1 variants={itemVariants} className="font-display font-extrabold text-[clamp(3rem,10vw,7rem)] leading-[0.95] tracking-tighter bg-gradient-to-r from-[#4ade80] via-[#22d3ee] to-[#818cf8] bg-clip-text text-transparent">Rodrigue</motion.h1>
        ...
        <motion.p variants={itemVariants} className="text-base md:text-lg text-white/55 leading-relaxed max-w-lg mx-auto mb-10">
          Je conçois des applications <strong className="text-white/85 font-semibold">web & mobile</strong> sur mesure - modernes, performantes et pensées pour vos utilisateurs.
        </motion.p>
        ...
        <motion.div variants={itemVariants} className="flex flex-wrap items-center justify-center gap-4 mb-12">
          ...
          <motion.a whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }} href="https://www.canva.com/design/DAGyrVVfur8/6VgU25AyyjxvB55KPsyV1Q/view" target="_blank" rel="noopener noreferrer"
            className="inline-flex items-center gap-2 px-7 py-3.5 rounded-full bg-white/[0.06] text-white font-medium text-sm border border-white/[0.12] transition-all hover:bg-white/[0.1]">
        ...
        <motion.div variants={itemVariants} className="flex justify-center gap-4">
          {[
            { href: 'https://github.com/nkgr01', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22"/></svg>, label: 'GitHub' },
            { href: 'https://www.linkedin.com/in/rodrigue-n-guessan/', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"/><rect x="2" y="9" width="4" height="12"/><circle cx="4" cy="4" r="2"/></svg>, label: 'LinkedIn' },
            { href: 'mailto:kouameguyrodriguen@gmail.com', icon: <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"/></svg>, label: 'Email' },
          ].map(({ href, icon, label }) => (
            <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label}
              whileHover={{ y: -3, backgroundColor: 'rgba(74, 222, 128, 0.1)', borderColor: 'rgba(74, 222, 128, 0.3)', color: '#4ade80' }}
              className="w-11 h-11 rounded-full flex items-center justify-center bg-white/[0.05] border border-white/[0.1] text-white/50 transition-all">{icon}</motion.a>
          ))}
        </motion.div>
      </motion.div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.5 }} onClick={() => scrollToSection('about')}
        className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 cursor-pointer text-white/30 hover:text-white/70 transition-colors">
